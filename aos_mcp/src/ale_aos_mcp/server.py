...
parser.add_argument(
	"--aos-ssh-url",
	type=str,
	default=os.environ.get("ALE_AOS_MCP_SSH_URL", "http://localhost:8110"),
	help="AOS Server URL",
)
parser.add_argument(
	"--transport",
	type=str,
	default=os.environ.get("ALE_AOS_MCP_TRANSPORT", "stdio"),
	help="transport method (stdio, streamable-http, sse, etc.)",
)
parser.add_argument(
	"--port", type=int, default=os.environ.get("ALE_AOS_MCP_PORT", "8000"), help="port for AOS MCP server"
)
parser.add_argument(
	"--aos-tools-file", type=str, default=os.environ.get("ALE_AOS_MCP_TOOLS_FILE", ""), help="mcp Tools file"
)
parser.add_argument(
	"--log-level",
	type=str,
	default=os.environ.get("ALE_AOS_MCP_LOG_LEVEL", "INFO"),
...
@mcp.tool()
async def test_elicit(ctx: Context, name: str = "", email: str = "") -> str:
	"""test elicit function to get user information."""
	await ctx.info("Processing test_elicit")
	...
	try:
		...
		return "Elicitation not supported. Please provide 'name' and 'email' parameters."
...
async def execute_command(
	...
	Command list :
		...
		- `show chassis`: Displays the basic configuration and status information for the switch chassis (serial
		number, model name, mac address, part number).
		- `show virtual-chassis topology`: Provide a detailed status of the virtual chassis topology.
		- `show vlan <vlanid>`: Displays VLAN information for the switch. if <vlanid> parameter is present, displays
		information for a specific VLAN otherwise display all vlans.
		- `show vlan <vlanid> members`: Displays VLAN port associations (VPAs) for all VLANs or a specific VLAN if
		<vlanid> parameter is present, for all ports.
		- `show vlan <vlanid> members port <portid>`: Displays VLAN port associations (VPAs) for all VLANs, a
		specific VLAN if <vlanid> parameter is present  , for a specific port with parameter <portid> as format
		chassis/slot/port .
		...
		- `ping <addr>`: Tests whether an IP destination can be reached from the local switch. This command sends an
		ICMP echo request to a destination and then waits for a reply. To ping a destination, enter the ping
		command and enter either the IP address or hostname of the destination. <addr> is hostname or IP address
		parameter to ping.
		- `traceroute <addr>`: Traces the route to a destination IP address or hostname. This command sends a series
		of ICMP echo requests to the destination and then waits for replies. The command displays the IP address of
		each hop along the route to the destination. <addr> is hostname or IP address parameter to traceroute.
		- `show interfaces <port>`: Displays the switch interfaces. Optional parameter <port> is the port to display
...
async def aos_commands() -> str:
	return "display all commands available for switches"
...
def main():
	...
	if not args.aos_ssh_url.startswith(("http://", "https://")):
		args.aos_ssh_url = "http://" + args.aos_ssh_url
